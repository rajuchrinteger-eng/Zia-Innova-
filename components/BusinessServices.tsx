    ...
    <section className="py-24 bg-white relative overflow-hidden">
      <div className="container mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-20 animate-fade-up">
          <h2 className="text-4xl font-bold text-slate-900 mb-6 tracking-tight">Complete Business Support</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {businessServices.map((s, i) => (
            <div 
              key={i} 
              className="group relative rounded-[30px] overflow-hidden cursor-pointer shadow-sm hover:shadow-xl transition-all duration-300 animate-fade-up aspect-[4/5]"
            ...
            >
              ...
              <div className="absolute top-5 right-5 w-10 h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white border border-white/20">
                <i className={`fa-solid ${s.icon} text-sm`}></i>
              </div>

              <div className="absolute bottom-0 left-0 p-6 w-full">
                <h4 className="text-lg font-bold text-white mb-1">{s.title}</h4>
                <p className="text-slate-300 text-xs leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-2 group-hover:translate-y-0">
                  {s.desc}
                </p>
              </div>
            </div>
          ...
          <div 
             onClick={() => navigate('#service-business')}
             className="group relative rounded-[30px] overflow-hidden cursor-pointer bg-slate-100 border border-slate-200 flex flex-col items-center justify-center aspect-[4/5] hover:bg-slate-200 transition-colors"
          >
             <div className="w-16 h-16 rounded-full bg-white shadow-sm flex items-center justify-center text-slate-900 text-xl mb-4 group-hover:scale-110 transition-transform">
               <i className="fa-solid fa-arrow-right"></i>
             </div>
             <span className="font-bold text-slate-900">View All Services</span>
          </div>
        </div>
      </div>
    </section>
